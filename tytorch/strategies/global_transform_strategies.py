...
class SequenceTensorSplitStrategy(GlobalTransformStrategy):
    def __init__(
        self, test_frac: float = 0.2, valid_frac: float = 0.1, shuffle_all: bool = True
    ) -> None:
        """
        Args:
            test_frac: Proportion of the dataset to include in the test split.
    ...
    def apply(self, pipeline: Any) -> Optional[torch.Tensor]:
        ...
        if pipeline.labels is not None:
            ...
            traintest_data_split = [pipeline.data[i] for i in train_indices]
            traintest_labels_split = [pipeline.labels[i] for i in train_indices]
            ...
            # Store the splits in the pipeline
            pipeline.train_data = [traintest_data_split[i] for i in train_indices]
            pipeline.train_labels = [traintest_labels_split[i] for i in train_indices]
            pipeline.val_data = [traintest_data_split[i] for i in val_indices]
            pipeline.val_labels = [traintest_labels_split[i] for i in val_indices]
            pipeline.test_data = [pipeline.data[i] for i in test_indices]
            pipeline.test_labels = [pipeline.labels[i] for i in test_indices]
        else:
            raise ValueError("Cannot split dataset without explicit labels.")
